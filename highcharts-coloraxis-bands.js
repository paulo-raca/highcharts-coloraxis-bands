    ...
    function remapStopRange(stops, oldMin, oldMax, newMin, newMax) {
        ...
        if (stops.length > 1) {
            if (stops[0][0] < oldMin) {
                stops[0] = [
                    oldMin,
                    stops[0].color.tweenTo(stops[1].color, (oldMin - stops[0][0]) / (stops[1][0] - stops[0][0])),
                ];
            ...
            if (stops[stops.length - 1][0] > oldMax) {
                stops[stops.length - 1] = [
                    ...
                    stops[stops.length - 2].color.tweenTo(
                        ...
                        (oldMax - stops[stops.length - 2][0]) /
                            (stops[stops.length - 1][0] - stops[stops.length - 2][0])
                    ),
                ];
    ...
    function simplifyStops(stops) {
        ...
        for (var i = 0; i < stops.length; i++) {
            ...
            if (ret.length >= 2) {
                ...
                if (
                    (middle[0] === left[0] && middle[0] === right[0]) ||
